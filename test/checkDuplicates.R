    ...
    for (col in c("visit", "week")) {
      if (col %in% colnames(df)) {
        ...
        if (nrow(df_dup_ids) > 0) {
          df_duplicates_qualtrics <- df %>% filter((.[[identifier]] %in% df_dup_ids[[identifier]]) & (.[[col]] %in% df_dup_ids[[col]]))
          
          # Check for duplicates and create a CSV file if found
          if (nrow(df_duplicates_qualtrics) > 0) {
            
            # export
            duplicate_extract <- paste0("qualtrics_duplicates_", measure_alias)
            
            # Use your custom function to create a CSV file
            createCsv(df_duplicates_qualtrics, paste0("qualtrics_duplicates_", measure_alias))
            
            # Wrap the test_that in a try-catch block
            tryCatch({
              test_that("Check for Qualtrics duplicates", {
                testthat::expect_true(nrow(df_duplicates_qualtrics) == 0, 
                                      info = paste("DATA ERROR: Duplicates detected in '", measure_alias, "': ", 
                                                   "Offending IDs: ", toString(unique(df_duplicates_qualtrics[[identifier]]))))
              })
            }, error = function(e) {
              # Convert the duplicate details into a readable format
              duplicates_summary <- toString(unique(df_duplicates_qualtrics[[identifier]]))
            ...
            View(df_duplicates_qualtrics)
          }
        }
      }
    }
  }
  if (measure_type == "task") {
    if (!(identifier %in% colnames(df))) {
      stop("Please provide a valid identifier: src_subject_id, workerId, PROLIFIC_PID")
    }
    ...
    for (col in c("visit", "week")) {
      if (col %in% colnames(df)) {
        ...
        if (nrow(df_dup_ids) > 0) {
          df_duplicates_task <- df %>% filter((.[[identifier]] %in% df_dup_ids[[identifier]]) & (.[[col]] %in% df_dup_ids[[col]]))
          
          # Check for duplicates and create a CSV file if found
          if (nrow(df_duplicates_task) > 0) {
            
            # export
            duplicate_extract <- paste0("task_duplicates_", measure_alias)
            
            # Use your custom function to create a CSV file
            createCsv(df_duplicates_task, paste0("task_duplicates_", measure_alias))
            
            # Wrap the test_that in a try-catch block
            tryCatch({
              test_that("Check for task duplicates", {
                testthat::expect_true(nrow(df_duplicates_task) == 0, 
                                      info = paste("DATA ERROR: Duplicates detected in '", measure_alias, "': ", 
                                                   "Offending IDs: ", toString(unique(df_duplicates_task[[identifier]]))))
              })
            }, error = function(e) {
              # Convert the duplicate details into a readable format
              duplicates_summary <- toString(unique(df_duplicates_task[[identifier]]))
              message(paste("Error in testing for duplicates in '", measure_alias, "': ", e$message, 
                            "\nOffending IDs: ", duplicates_summary,
                            "\nDetails exported to ", paste0("./tmp/",duplicate_extract,".csv")))
            })
  ...
  if (measure_type == "redcap") {
    ...
    for (col in c("visit", "week")) {
      if (col %in% colnames(df)) {
        ...
        if (nrow(df_dup_ids) > 0) {
          df_duplicates_redcap <- df %>% filter((.[[identifier]] %in% df_dup_ids[[identifier]]) & (.[[col]] %in% df_dup_ids[[col]]))
          
          # Check for duplicates and create a CSV file if found
          if (nrow(df_duplicates_redcap) > 0) {
            
            # export
            duplicate_extract <- paste0("redcap_duplicates_", measure_alias)
            
            # Use your custom function to create a CSV file
            createCsv(df_duplicates_redcap, paste0("redcap_duplicates_", measure_alias))
            
            # Wrap the test_that in a try-catch block
            tryCatch({
              test_that("Check for redcap duplicates", {
                testthat::expect_true(nrow(df_duplicates_redcap) == 0, 
                                      info = paste("DATA ERROR: Duplicates detected in '", measure_alias, "': ", 
                                                   "Offending IDs: ", toString(unique(df_duplicates_redcap[[identifier]]))))
              })
            }, error = function(e) {
              # Convert the duplicate details into a readable format
              duplicates_summary <- toString(unique(df_duplicates_redcap[[identifier]]))
              message(paste("Error in testing for duplicates in '", measure_alias, "': ", e$message, 
                            "\nOffending IDs: ", duplicates_summary,
                            "\nDetails exported to ", paste0("./tmp/",duplicate_extract,".csv")))
            })
